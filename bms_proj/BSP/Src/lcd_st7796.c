/**
 * lcd_st7796.c — Sitronix ST7796S 3.5" 320×480 SPI 驱动
 *
 * 初始化序列照搬 ST7796_TN_Code.txt (鸿讯电子提供), 仅改 MADCTL 为 0x48
 * (与 datasheet 默认一致, BGR=1 与 IPS 屏匹配, 若颜色反相把 BGR 位翻一下)。
 */
...
bool lcd_init(void)
{
    ...
    lcd_cmd(0xF0); lcd_data(0xC3);                /* Command Set Control 1 (unlock) */
    lcd_cmd(0xF0); lcd_data(0x96);                /* Command Set Control 2 */
    lcd_cmd(0x36); lcd_data(0xE8);                /* MADCTL: MV+MX+MY+BGR — 横屏 480×320 */
    lcd_cmd(0x3A); lcd_data(0x55);                /* COLMOD: 16-bit RGB565 */
    lcd_cmd(0xB4); lcd_data(0x01);                /* Inversion */
    lcd_cmd(0xB7); lcd_data(0xC6);                /* Entry Mode Set */

    lcd_cmd(0xE8); {
        const uint8_t d[8] = {0x40,0x8A,0x00,0x00,0x29,0x19,0xA5,0x33};
        lcd_data_buf(d, 8);                       /* Display Output Ctrl */
    }
